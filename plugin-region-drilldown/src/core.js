...
const DEFAULT_OPTIONS = {
  ...
  controlsTitle: "行政区下钻",
  backText: "返回上一级",
  worldViewHeight: 22500000,
  regionViewHeight: 4200000,
  cameraDuration: 1.15,
  minMarkerSize: 62,
  maxMarkerSize: 92,
  layoutPadding: 8,
...
function normalizeCenter(region) {
  ...
  if (Array.isArray(raw) && raw.length >= 2) {
    const longitude = numberOrNull(raw[0]);
    const latitude = numberOrNull(raw[1]);
    return longitude === null || latitude === null ? null : [longitude, latitude];
  }
  if (raw && typeof raw === "object") {
    const longitude = firstFinite([raw.longitude, raw.lon, raw.lng, raw.x]);
    const latitude = firstFinite([raw.latitude, raw.lat, raw.y]);
    return longitude === null || latitude === null ? null : [longitude, latitude];
  }
...
function normalizeRegionPayload(payload, fallbackLevel) {
  const list = Array.isArray(payload)
    ? payload
    : toArray(payload && (payload.items || payload.regions || payload.data || payload.children));
...
function formatCount(value) {
  ...
  if (number >= 100000000) {
    return `${(number / 100000000).toFixed(number >= 1000000000 ? 0 : 1)}亿`;
  }
  if (number >= 10000) {
...
function worldToWindow(Cesium, viewer, position) {
...
class RegionDrilldownController {
  constructor(Cesium, viewer, rawOptions) {
    if (!Cesium) {
    ...
    this.dataSource = new Cesium.CustomDataSource("行政区圆圈下钻");
  ...
  async activateRegion(region) {
    ...
    this.setStatus(`${region.name} · ${formatCount(region.datasetCount)} 集`, false);
  ...
  renderRegions(regions) {
    ...
    regions.forEach((region) => {
      ...
      const position = this.Cesium.Cartesian3.fromDegrees(region.center[0], region.center[1], 9000);
  ...
  flyToWorld() {
    ...
    this.viewer.camera.flyTo({
      destination: Cesium.Cartesian3.fromDegrees(104, 24, this.options.worldViewHeight),
  ...
  flyToRegion(region) {
    ...
    if (region.center) {
      ...
      const maxSpan = region.bounds
        ? Math.max(region.bounds[2] - region.bounds[0], region.bounds[3] - region.bounds[1])
        : 8;
      const height = Math.max(600000, Math.min(this.options.regionViewHeight, maxSpan * 150000));
      this.viewer.camera.flyTo({
...
export function createRegionDrilldownCore(Cesium, viewer, options) {
  return new RegionDrilldownController(Cesium, viewer, options);
}
